<?php
/**
 * Plugin Name: SMX Plugin
 * Plugin URI: #
 * Description: Wordpress crud by Saikat
 * Version: 1.0
 * Author: Saikat
 * Author URI: #
...
class WP_Job_Manager {
	...
	public function __construct() {
		// Define constants.
		define( 'SMX_VERSION', '1.32.3' );
		define( 'SMX_MINIMUM_WP_VERSION', '4.7.0' );
		define( 'SMX_PLUGIN_DIR', untrailingslashit( plugin_dir_path( __FILE__ ) ) );
		define( 'SMX_PLUGIN_URL', untrailingslashit( plugins_url( basename( plugin_dir_path( __FILE__ ) ), basename( __FILE__ ) ) ) );
		define( 'SMX_PLUGIN_BASENAME', plugin_basename( __FILE__ ) );

		// Includes.
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-install.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-post-types.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-ajax.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-shortcodes.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-api.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-forms.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-geocode.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-blocks.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-cache-helper.php';
		include_once SMX_PLUGIN_DIR . '/includes/helper/class-smx-helper.php';
		include_once SMX_PLUGIN_DIR . '/includes/abstracts/abstract-smx-email.php';
		include_once SMX_PLUGIN_DIR . '/includes/abstracts/abstract-smx-email-template.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-email-notifications.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-data-exporter.php';

		/**
		 * This custom REST API implementation is deprecated and will be removed in 1.33.0.
		 *
		 * @see WP_Job_Manager::rest_api()
		 * @see https://github.com/Automattic/smx/issues/1625
		 */
		if ( defined( 'WPJM_REST_API_ENABLED' ) && WPJM_REST_API_ENABLED ) {
			trigger_error( esc_html__( 'Constant `WPJM_REST_API_ENABLED` and custom REST API implementation is deprecated and will be removed in 1.33.0. Please use standard WP Core\'s implementation.', 'smx' ) );
			add_action( 'rest_api_init', array( $this, 'rest_api' ) );
		}

		if ( is_admin() ) {
			include_once SMX_PLUGIN_DIR . '/includes/admin/class-smx-admin.php';
		}

		// Load 3rd party customizations.
		include_once SMX_PLUGIN_DIR . '/includes/3rd-party/3rd-party.php';
	...
	public function updater() {
		if ( version_compare( SMX_VERSION, get_option( 'wp_job_manager_version' ), '>' ) ) {
			WP_Job_Manager_Install::install();
			flush_rewrite_rules();
		}
	}
	...
	public function add_privacy_policy_content() {
		...
		$content = sprintf(
			...
			__( 'This site adds the following cookies to help users resume job submissions that they 
				have started but have not completed: %1$s and %2$s', 'smx'
			),
			'<code>smx-submitting-job-id</code>', '<code>smx-submitting-job-key</code>'
		);
	...
	public function load_plugin_textdomain() {
		load_textdomain( 'smx', WP_LANG_DIR . '/smx/smx-' . apply_filters( 'plugin_locale', get_locale(), 'smx' ) . '.mo' );
		load_plugin_textdomain( 'smx', false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' );
	}
	...
	public function rest_api() {
		_deprecated_function(
			...
			'1.32.0',
			esc_html__( 'Standard REST API implementation from WP core', 'smx' )
		);
		if ( null === $this->rest_api ) {
			include_once SMX_PLUGIN_DIR . '/includes/rest-api/class-smx-rest-api.php';
	...
	public function include_template_functions() {
		include_once SMX_PLUGIN_DIR . '/smx-deprecated.php';
		include_once SMX_PLUGIN_DIR . '/smx-functions.php';
		include_once SMX_PLUGIN_DIR . '/smx-template.php';
	}
	...
	public function widgets_init() {
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-widget.php';
		include_once SMX_PLUGIN_DIR . '/includes/widgets/class-smx-widget-recent-jobs.php';
		include_once SMX_PLUGIN_DIR . '/includes/widgets/class-smx-widget-featured-jobs.php';
	}
	...
	public function usage_tracking_init() {
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-usage-tracking.php';
		include_once SMX_PLUGIN_DIR . '/includes/class-smx-usage-tracking-data.php';
	...
	public function cleanup_job_posting_cookies() {
		if ( isset( $_COOKIE['smx-submitting-job-id'] ) ) {
			setcookie( 'smx-submitting-job-id', '', 0, COOKIEPATH, COOKIE_DOMAIN, false );
		}
		if ( isset( $_COOKIE['smx-submitting-job-key'] ) ) {
			setcookie( 'smx-submitting-job-key', '', 0, COOKIEPATH, COOKIE_DOMAIN, false );
		}
	}
	...
	public static function register_select2_assets() {
		wp_register_script( 'select2', SMX_PLUGIN_URL . '/assets/js/select2/select2.full.min.js', array( 'jquery' ), '4.0.5' );
		wp_register_style( 'select2', SMX_PLUGIN_URL . '/assets/js/select2/select2.min.css', array(), '4.0.5' );
	}
	...
	public function frontend_scripts() {
		...
		$ajax_data        = array(
			'ajax_url'                => $ajax_url,
			'is_rtl'                  => is_rtl() ? 1 : 0,
			'i18n_load_prev_listings' => __( 'Load previous listings', 'smx' ),
		);
		...
		/**
		 * Set the constant `SMX_DISABLE_CHOSEN_LEGACY_COMPAT` to true to test for future behavior once
		 * this legacy code is removed and `chosen` is no longer packaged with the plugin.
		 */
		if ( ! defined( 'SMX_DISABLE_CHOSEN_LEGACY_COMPAT' ) || ! SMX_DISABLE_CHOSEN_LEGACY_COMPAT ) {
			...
			if ( ! wp_script_is( 'chosen', 'registered' ) ) {
				wp_register_script( 'chosen', SMX_PLUGIN_URL . '/assets/js/jquery-chosen/chosen.jquery.min.js', array( 'jquery' ), '1.1.0', true );
				wp_register_style( 'chosen', SMX_PLUGIN_URL . '/assets/css/chosen.css', array(), '1.1.0' );
			}
		...
		if ( apply_filters( 'job_manager_enhanced_select_enabled', $enhanced_select_used_on_page ) ) {
			...
			wp_register_script( 'smx-term-multiselect', SMX_PLUGIN_URL . '/assets/js/term-multiselect.min.js', array( 'jquery', 'select2' ), SMX_VERSION, true );
			wp_register_script( 'smx-multiselect', SMX_PLUGIN_URL . '/assets/js/multiselect.min.js', array( 'jquery', 'select2' ), SMX_VERSION, true );
		...
		if ( job_manager_user_can_upload_file_via_ajax() ) {
			wp_register_script( 'jquery-iframe-transport', SMX_PLUGIN_URL . '/assets/js/jquery-fileupload/jquery.iframe-transport.js', array( 'jquery' ), '1.8.3', true );
			wp_register_script( 'jquery-fileupload', SMX_PLUGIN_URL . '/assets/js/jquery-fileupload/jquery.fileupload.js', array( 'jquery', 'jquery-iframe-transport', 'jquery-ui-widget' ), '9.11.2', true );
			wp_register_script( 'smx-ajax-file-upload', SMX_PLUGIN_URL . '/assets/js/ajax-file-upload.min.js', array( 'jquery', 'jquery-fileupload' ), SMX_VERSION, true );
			...
			wp_localize_script(
				'smx-ajax-file-upload',
				'job_manager_ajax_file_upload',
				array(
					'ajax_url'               => $ajax_url,
					'js_field_html_img'      => esc_js( str_replace( "\n", '', $js_field_html_img ) ),
					'js_field_html'          => esc_js( str_replace( "\n", '', $js_field_html ) ),
					'i18n_invalid_file_type' => __( 'Invalid file type. Accepted types:', 'smx' ),
				)
			);
		}

		wp_register_script( 'jquery-deserialize', SMX_PLUGIN_URL . '/assets/js/jquery-deserialize/jquery.deserialize.js', array( 'jquery' ), '1.2.1', true );
		wp_register_script( 'smx-ajax-filters', SMX_PLUGIN_URL . '/assets/js/ajax-filters.min.js', $ajax_filter_deps, SMX_VERSION, true );
		wp_register_script( 'smx-job-dashboard', SMX_PLUGIN_URL . '/assets/js/job-dashboard.min.js', array( 'jquery' ), SMX_VERSION, true );
		wp_register_script( 'smx-job-application', SMX_PLUGIN_URL . '/assets/js/job-application.min.js', array( 'jquery' ), SMX_VERSION, true );
		wp_register_script( 'smx-job-submission', SMX_PLUGIN_URL . '/assets/js/job-submission.min.js', array( 'jquery' ), SMX_VERSION, true );
		wp_localize_script( 'smx-ajax-filters', 'job_manager_ajax_filters', $ajax_data );
		wp_localize_script(
			'smx-job-dashboard',
			'job_manager_job_dashboard',
			array(
				'i18n_confirm_delete' => __( 'Are you sure you want to delete this listing?', 'smx' ),
			)
		);
		...
		if ( apply_filters( 'job_manager_enqueue_frontend_style', is_wpjm() ) ) {
			wp_enqueue_style( 'smx-frontend', SMX_PLUGIN_URL . '/assets/css/frontend.css', array(), SMX_VERSION );
		} else {
			wp_register_style( 'smx-job-listings', SMX_PLUGIN_URL . '/assets/css/job-listings.css', array(), SMX_VERSION );
		}
	}
}
